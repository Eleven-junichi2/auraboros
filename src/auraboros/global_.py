...
def _fix_dislodge_between_givenpos_and_drawpos(func_which_has_pos_as_return):
    """decorator"""
...
def _decorate_draw(func, pixel_scale):
    ...
    def wrapper(*args, **kwargs):
        new_args = list(
            map(lambda arg: list(arg) if isinstance(arg, tuple) else arg, args)
        )
        let_fix_pos = False
        is_kwargs_set = False
        if func.__name__ == "rect" or func.__name__ == "ellipse":
            ...
            if "rect" in kwargs:
                if isinstance(
                    kwargs["rect"][0], IntOfMousePosForDrawToScaledScrn
                ):
                    let_fix_pos = True
                    is_kwargs_set = True
            ...
            if "rect" in kwargs:
                if isinstance(
                    kwargs["rect"][1], IntOfMousePosForDrawToScaledScrn
                ):
                    let_fix_pos = True
                    is_kwargs_set = True
            if let_fix_pos:
        ...
        new_args = list(
            map(
                lambda arg: tuple(arg) if isinstance(arg, list) else arg,
                new_args,
            )
        )
