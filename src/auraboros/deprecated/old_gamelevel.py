...
class Level:
    def __init__(self, levelfile_dir, scene=None):
        ...
        self.level_raw_pattern_data = open_json_file(levelfile_dir / "patterns.json")
        ...
        self.bg_surf = pygame.surface.Surface(
            (Global.w_size[0], Global.w_size[1] * 2)
        )
    ...
    def enemies(self) -> EntityList[Enemy]:
        enemy_list = [enemy for enemy in self.entities if isinstance(enemy, Enemy)]
        return enemy_list

    def deadly_obstacles(self) -> EntityList[DeadlyObstacle]:
        deadly_obstacle_list = [
            deadly_obstacle
            for deadly_obstacle in self.entities
    ...
    def run_level(self, dt):
                ...
                for i in range(2):
                    if isinstance(data["pos"][i], str):
                        if data["pos"][i] == "random":
                            pos[i] = randint(0, Global.w_size[i])
                        if data["pos"][i] == "right":
                            pos[i] = Global.w_size[i] - enemy.rect.width
                    else:
                        pos[i] = data["pos"][i]
                enemy.x, enemy.y = pos
        ...
        self.elapsed_time_in_level += 1 * dt * Global.TARGET_FPS

    def process_collision(
        self, player_entities: Iterable[Entity], weapon_entities: Iterable[Entity]
    ) -> bool:
    ...
    def read_tagged_level_data(self):
        ...
        for data in self.level_raw_data:
            if isinstance(data, list):
                without_str = list(
                    filter(lambda item: not isinstance(item, list), self.level_raw_data)
                )
    ...
    def clear_enemies_off_screen(self):
        [
            entity.remove_from_container()
            for entity in self.enemies()
            if Global.w_size[1] < entity.y
            or Global.w_size[0] < entity.x
            or entity.x < 0
            or entity.y < 0
        ]

    def stop_entity_from_moving_off_screen(self, entity: Entity):
        if Global.w_size[1] < entity.y + entity.rect.height:
            entity.arrow_of_move.unset(Arrow.down)
        if Global.w_size[0] < entity.x + entity.rect.width:
    ...
    def set_background(self):
        [
            self.bg_surf.fill(
                ...
                (
                    (randint(0, Global.w_size[0]), randint(0, Global.w_size[1] * 2)),
                    (1, 1),
                ),
            )
    ...
    def set_background_for_scroll(self):
        ...
        new_background.blit(
            self.bg_surf, (0, Global.w_size[1], Global.w_size[0], Global.w_size[1])
        )
        randomize_density = randint(
            -self.density_of_stars_on_bg // 2, self.density_of_stars_on_bg // 2
        )
        [
            new_background.fill(
                (randint(0, 255), randint(0, 255), randint(0, 255)),
                (
                    (randint(0, Global.w_size[0]), randint(0, Global.w_size[1])),
                    (1, 1),
                ),
            )
    ...
    def scroll(self, dt):
        for enemy in self.enemies():
            enemy.y += self.scroll_speed * 1.25 * dt * Global.TARGET_FPS
        self.bg_scroll_y += self.scroll_speed * dt * Global.TARGET_FPS
        if self.bg_scroll_y > Global.w_size[1]:
            self.bg_scroll_y = 0
